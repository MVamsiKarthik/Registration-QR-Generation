...
def is_rfc1918_private_ip(ip_text):
    try:
        ip = ipaddress.ip_address(clean_text(ip_text))
    except ValueError:
        return False
...
st.caption(f"Active QR base URL: {QR_BASE_URL}")

host_only = ""
try:
    host_only = (QR_BASE_URL.split("://", 1)[1].split("/", 1)[0]).split(":", 1)[0]
except IndexError:
    host_only = ""

if settings.get("auto_detect_ip", True) and host_only and not is_rfc1918_private_ip(host_only):
    st.warning(
        "Auto-detected IP is not a standard private LAN address. "
        "Set Admin Settings -> Manual QR Base URL to your current laptop IPv4 "
        "(for example http://192.168.x.x:5000), then click Update All QRs."
    )

host_is_local_target = host_only in {"127.0.0.1", "localhost"} or is_rfc1918_private_ip(host_only)
if host_is_local_target and not is_local_port_open(FLASK_PORT):
    st.error(
        f"QR links open on port {FLASK_PORT}, but web_app.py is not running. "
        "Start it with: python web_app.py"
    )
